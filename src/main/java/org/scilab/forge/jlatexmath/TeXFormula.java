...
public class TeXFormula {
    ...
    public static double PIXELS_PER_POINT = 12;
    ...
    public static void setDefaultDPI() {
        if (null != Screen.getPrimary()) {
            setDPITarget((double) Screen.getPrimary().getDpi());
        }
    }
     ...
     *                              a valid unit
     */
    public TeXFormula addStrut(int unit, double width, double height, double depth)
     ...
     *                              a valid unit
     */
    public TeXFormula addStrut(int widthUnit, double width, int heightUnit,
                               double height, int depthUnit, double depth) throws InvalidUnitException {
    ...
    public TeXIcon createTeXIcon(int style, double size, int widthUnit, double textwidth, int align, int interlineUnit, double interline) {
        return createTeXIcon(style, size, 0, widthUnit, textwidth, align, interlineUnit, interline);
    }

    public TeXIcon createTeXIcon(int style, double size, int type, int widthUnit, double textwidth, int align, int interlineUnit, double interline) {
    ...
    public void createImage(String format, int style, double size, String out, Paint bg, Paint fg, boolean transparency) {
        TeXIcon icon = createTeXIcon(style, size);
        icon.setInsets(new Insets(1, 1, 1, 1));
        int w = icon.getIconWidth(), h = icon.getIconHeight();

        SnapshotParameters sp = new SnapshotParameters();
        if (transparency) sp.setFill(Color.TRANSPARENT);
        Canvas canvas = new Canvas(w, h);

        icon.setForeground(fg);
        icon.paintInCanvas(canvas, 0, 0, fg, bg);

        WritableImage image = new WritableImage(w, h);
        canvas.snapshot(sp, image);

        File imout = new File(out);

        try {
            ImageIO.write(SwingFXUtils.fromFXImage(image, null), format, imout);
        } catch (IOException e) {
            e.printStackTrace();
        }
