...
public class PredefMacros {
    ...
    public static final Atom GeoGebra_macro(final TeXParser tp, final String[] args) throws ParseException {
        ...
        tf.add("\\mathsf{Gebra}");
        return new ColorAtom(tf.root, null, new Color(102, 102, 102, 1));
    }
    ...
    public static final Atom definecolor_macro(final TeXParser tp, final String[] args) throws ParseException {
        ...
        if ("gray".equals(args[2])) {
            ...
            double r = Double.parseDouble(stok.nextToken().trim());
            double g = Double.parseDouble(stok.nextToken().trim());
            double b = Double.parseDouble(stok.nextToken().trim());
            ...
            for (int i = 0; i < 4; i++)
                cmyk[i] = Double.parseDouble(stok.nextToken().trim());
            double k = 1 - cmyk[3];
            color = new Color(k * (1 - cmyk[0]), k * (1 - cmyk[1]), k * (1 - cmyk[2]), 1);
        } else
    ...
    public static final Atom jlmText_macro(final TeXParser tp, final String[] args) throws ParseException {
        return new JavaFontRenderingAtom(args[1], FontWeight.NORMAL);
    }

    public static final Atom jlmTextit_macro(final TeXParser tp, final String[] args) throws ParseException {
        return new JavaFontRenderingAtom(args[1], FontWeight.NORMAL, FontPosture.ITALIC);
    }
    ...
    public static final Atom jlmTextitbf_macro(final TeXParser tp, final String[] args) throws ParseException {
        return new JavaFontRenderingAtom(args[1], FontWeight.BOLD, FontPosture.ITALIC);
    }

    public static final Atom DeclareMathSizes_macro(final TeXParser tp, final String[] args) throws ParseException {
        DefaultTeXFont.setMathSizes(Double.parseDouble(args[1]), Double.parseDouble(args[2]), Double.parseDouble(args[3]), Double.parseDouble(args[4]));
        return null;
    }
